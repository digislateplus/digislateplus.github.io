...
The DigiSlatePlus has two operating states. If an external time code is fed in and the slate is switched on, it starts in read mode. The time code is read, shown on the LED display and the time in the RTC is updated.
...
The original hardware is adapted in some respects to enable the functions. The changes are listed in doc directory.

The code is completely written in C++ and is available in a first stable version. Yet some details are still in development. 
